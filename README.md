...
Here, ```keywords``` filters articles that contain at least one of the specified keywords (max 2 words per keyword recommended). This filtering is applied **before** multiprocessing. The ```text_condition``` function keeps only articles satisfying the given condition, and is applied **after** multiprocessing.  
